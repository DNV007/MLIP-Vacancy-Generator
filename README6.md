...
The script asks you to choose one of five modes.
...
### `migration`
Generate vacancy-defect structures together with simple migration-path images.

Use this when:
- you want to study Mg hop families around a vacancy
- you want start, intermediate, and end images for candidate diffusion paths
- you want a curved, saddle-like interpolated image in addition to the linear path

This mode:
- starts from the symmetry-unique vacancy combinations by default
- enumerates same-species hop candidates near each vacancy
- builds source-to-vacancy assignments
- writes path images with a configurable perpendicular saddle offset
...
### Prompt 10
```text
Choose mode [raw/unique/seeded/mlip/migration, default: mlip]:
```
What to type:
- `raw`
- `unique`
...
- `migration` for vacancy defects plus candidate hop-path images
...
## 5.8 Additional prompts only in `migration` mode

If mode = `migration`, you will see:
...
### Prompt 12
```text
Migration target family mode [all/direct/tetrahedral/octahedral, default: all]:
```
...
### Prompt 14
```text
Maximum source candidates per vacancy [6]:
```

### Prompt 15
```text
Maximum source-vacancy assignments per vacancy combo [5]:
```

### Prompt 16
```text
Number of images along each migration path [5]:
```
...
### Prompt 18
```text
Write both + and - saddle-side images? [y/n, default: n]:
```

The generated path images include:
- `path_start`
- `path_image`
- `path_saddle`
- `path_end`
- one file family per target mode, e.g. `direct`, `tetrahedral`, or `octahedral`
...
7. random seed
8. balancing cap questions
9. write base structures?
10. migration target family mode
11. maximum hop distance
12. max candidates per vacancy
13. max assignments per combo
14. number of path images
15. saddle shift amplitude
16. write both saddle-side images?
17. extxyz question
18. output directory
...
Defect recipe: Mg:1
Choose mode [raw/unique/seeded/mlip/migration, default: mlip]: mlip
...
Use `migration` when:
- you want vacancy structures plus plausible hop-path images
- you want to screen multiple source-to-vacancy assignments for the same vacancy cluster
- you want a simple saddle-like interpolated geometry for MLIP/NEB initialization
