...
def choose_perpendicular_unit(vector: np.ndarray,
    ...
    The function uses the Cartesian basis vectors as references and picks the
    cross product with the largest norm. This gives a stable, reproducible
    "sideways" direction for a saddle-like image.
    """
    norm = np.linalg.norm(vector)
    if norm < 1e-12:
...
def estimate_local_shell_normal(structure: Structure,
    ...
    This provides a reproducible geometric axis for tetrahedral/octahedral-like
    environments. The normal is the least-variance principal component of the
    nearest-neighbor displacement cloud around the vacancy site.
    """
...
def estimate_interstitial_target_point(
    ...
    The returned point is a geometric heuristic in Cartesian coordinates. It is
    not a relaxed saddle point, but it is intentionally shaped to resemble the
    local tetrahedral or octahedral pocket around the vacancy site.
    """
    ...
    if target_family == "tetrahedral":
        # Use the opposite of the average nearest-neighbor direction as a
        # simple pocket-centre estimate. This works reasonably well when the
        # vacancy sits near a four-fold local environment.
        ...
        # Use the least-variance principal axis of the local shell. This is a
        # stable geometric normal for a more open, octahedral-like pocket.
...
def classify_vacancy_topology(
    ...
    For single vacancies the label is ``single``. For pairs we report
    ``adjacent`` or ``separated``. For larger clusters we report the connected
    component sizes under a nearest-neighbor graph threshold.
    """
    if len(combo) == 1:
        return "single"
    ...
    # Build a simple graph over vacancies.
    n = len(combo)
    parent = list(range(n))
...
def find_migration_candidates(
    ...
    Candidates are restricted to atoms of the same species as the vacancy site
    and are sorted by the minimum-image distance to the vacancy.
    """
    ...
    if not candidates:
        # If the explicit cutoff misses everything, fall back to the nearest
        # same-species neighbors so the generator still produces a plausible
        # hop family for this lattice.
        for source_index, site in enumerate(parent_structure):
            if source_index == vacancy_index or source_index in forbidden:
                continue
            if site.specie.symbol != vacancy_symbol:
...
def enumerate_migration_assignments(
    ...
    Each assignment is a tuple of parent-site source indices in the same order
    as ``vacancy_indices``. The source indices are distinct, so one atom is not
    used twice in the same simultaneous hop pattern.
    """
...
def build_migration_image(
    ...
    for vacancy_index, source_index in zip(vacancy_indices, source_indices):
        ...
        new_structure.replace(defect_index, new_structure[defect_index].specie, new_cart, coords_are_cartesian=True)
...
def _write_migration_paths(
    ...
    For each vacancy combo, this enumerates source-to-vacancy assignments,
    builds a simple interpolated path, and emits start/intermediate/end images.
    The final state of a path is the vacancy configuration after the hop.
    """
    if path_images < 2:
...
def build_dataset_report(records: List[dict], summary: dict) -> dict:
    ...
    The report is intentionally metadata-only:
    - symmetry uniqueness is inferred from ``canonical_hash``
    - topology counts come from ``vacancy_topology``
    - migration families come from ``migration_target_family``

    SOAP pruning is summarized via the stored generation settings because the
    per-structure SOAP vectors are not retained in the dataset outputs.
    """
    report: dict = {
        ...
        "unique_canonical_hashes": len({r["canonical_hash"] for r in records if r.get("canonical_hash")}),
        "canonical_hash_counts": dict(Counter(r["canonical_hash"] for r in records if r.get("canonical_hash"))),
        "vacancy_topology_counts": dict(Counter(r.get("vacancy_topology", "") for r in records if r.get("vacancy_topology"))),
    ...
    if migration_records:
        report["migration_target_family_counts"] = dict(
            Counter(r.get("migration_target_family", "") for r in migration_records if r.get("migration_target_family"))
        )
        report["migration_path_index_counts"] = dict(
            Counter((r.get("migration_target_family", ""), str(r.get("path_index", ""))) for r in migration_records)
        )
    return report
...
def build_soap_distance_report(
    ...
    The rows are long-form pairwise distances between selected base defects.
    This is useful for checking that SOAP pruning actually retained structures
    that are far apart in the defect-local SOAP space.
    """
    ...
    if not (_DSCRIBE_AVAILABLE and _ASE_ADAPTOR_AVAILABLE):
        return [], {
            "soap_distance_pairs": 0,
            "soap_distance_min": None,
            "soap_distance_mean": None,
    ...
    defect_structures = [remove_sites_from_structure(working_structure, combo) for combo in base_combos]
...
def main() -> None:
    ...
    if use_soap_pruning and _DSCRIBE_AVAILABLE and _ASE_ADAPTOR_AVAILABLE:
    ...
    soap_report = build_dataset_report(metadata_records, summary)
    soap_report.update(soap_stats)
    save_json(soap_report, report_json_path)
    if use_soap_pruning and _DSCRIBE_AVAILABLE and _ASE_ADAPTOR_AVAILABLE:
        soap_report_data = {
